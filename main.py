...
def isWinning(m,p):
    ...
    if tmp[0]==[s,s,s] or tmp[1]==[s,s,s] or tmp[2]==[s,s,s]: # Check rows
        result = True
    if [tmp[0][0],tmp[1][0],tmp[2][0]]==[s,s,s] or [tmp[0][1],tmp[1][1],tmp[2][1]]==[s,s,s] or [tmp[0][2],tmp[1][2],tmp[2][2]]==[s,s,s]: # Check cols
        result = True
    if  [m[0][0],m[1][1],m[2][2]]==[s,s,s] or [m[0][2],m[1][1],m[2][0]]==[s,s,s]: #Check diagonals
        result = True
    return result    
...
def checkWin(s):
    ...
    if [game[0][0],game[0][1],game[0][2]]==[s,s,s] or [game[1][0],game[1][1],game[1][2]]==[s,s,s] or [game[2][0],game[2][1],game[2][2]]==[s,s,s]:
        result = True
    # Check Diagonal positions
    if  [game[0][0],game[1][1],game[2][2]]==[s,s,s] or [game[0][2],game[1][1],game[2][0]]==[s,s,s]:
        result = True
    return result
...
def checkXMoves():
    ...
    for lm in legalMoves:
        ...
        tmp[lm]="X"
        # Check rows 
        if tmp[0] in winningMoves or tmp[1] in winningMoves or tmp[2] in winningMoves:
            xMoves.append(lm)
        #Check cols
        if [tmp[0][0],tmp[0][1],tmp[0][2]] in winningMoves or [tmp[1][0],tmp[1][1],tmp[2][2]] in winningMoves or [tmp[2][0],tmp[2][1],tmp[2][2]] in winningMoves:
            xMoves.append(lm)
        #Check diagonals
        if [tmp[0][0],tmp[1][1],tmp[2][2]] in winningMoves or [tmp[0][2],tmp[1][1],tmp[2][0]]:
            xMoves.append(lm)
    return xMoves       
